...
- Read [the architecture reference](docs/agent-reference/site-architecture.md) before changing routes, templates, design primitives, or rendering. The per-file map is [key-files.md](docs/agent-reference/key-files.md), the 2026 rebrand cutovers are in [labs-shell.md](docs/agent-reference/labs-shell.md), and migration leftovers in [legacy-terminology.md](docs/agent-reference/legacy-terminology.md). The website-development skill routes to this and validation details. `docs/MIGRATION.md` and `docs/kitaru-seo-inventory.md` are historical/operational context, not architecture authority.
