...
<FeatureWithGraphic title="Caching that skips the expensive steps">
  ...
  <Fragment slot="bullets">
    ...
    - **Sandboxes:** An agent that runs generated code gets a `sandbox` stack component with local, Docker, Kubernetes, and Modal flavors: `Client().active_stack.sandbox.create_session()`, then `session.exec([...])`. Docker and Modal add snapshot and restore. Temporal leaves isolation up to your Activity.
  </Fragment>
</FeatureWithGraphic>
