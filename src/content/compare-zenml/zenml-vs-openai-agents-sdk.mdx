...
<FeatureWithGraphic title="Re-running without starting from step one">
  ...
  <Fragment slot="bullets">
    ...
    - **After a failure:** `zenml pipeline runs retry <run>` re-runs a failed run and the steps that completed come from cache, so the retry doesn't re-pay for the work above the failure. ZenML retries at step granularity. It won't resume a step part-way through its body.
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: "Isolated code execution for tool loops", zenml: true, competitor: "partial", note: "ZenML's sandbox is a stack component with local, Docker, Kubernetes and Modal flavors. The SDK's hosted code interpreter tool runs on OpenAI's side." },
...
| Code execution | Hosted code interpreter tool | Sandbox stack component (local, Docker, Kubernetes, Modal) |
