...
<FeatureWithGraphic title="Durable waits and dynamic graphs vs polyglot durable functions" reverse={true}>
  ...
  <Fragment slot="bullets">
    ...
    - **Sandboxes:** A `sandbox` stack component (local, Docker, Kubernetes and Modal flavors) gives an agent step an isolated session to run generated code in, and on the Docker and Modal flavors snapshot it and restore it later. Inngest has nothing like it. Fair's fair: throttling, concurrency keys and event subscriptions have no ZenML equivalent either.
  </Fragment>
</FeatureWithGraphic>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: "Resume a function mid-body from memoized steps", zenml: false, competitor: true, note: "ZenML retries at step granularity; on a retried dynamic run, completed steps are reused rather than re-executed." },
    ...
    { feature: "Event subscriptions as triggers", zenml: false, competitor: true, note: "ZenML pipelines are invoked, scheduled through the orchestrator or ZenML Pro, or deployed; they don't subscribe to external events, though ZenML Pro can trigger a pipeline when another run finishes." },
    { feature: "TypeScript and Go as well as Python", zenml: false, competitor: true, note: "ZenML is Python only." },
    { feature: "Open source, self-hostable", zenml: true, competitor: "partial", note: "ZenML is Apache 2.0. The Inngest server is SSPL with delayed Apache 2.0 publication, and its SDKs are Apache 2.0. Self-hosting is supported." },
  ]}
/>
