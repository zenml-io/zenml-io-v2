...
<div data-rt-embed-type="true"><div class="table-container">
  ...
  <title>LangGraph Plans Table</title>
  <style>
    /* Base table styling */
  ...
  <table class="comparison-table">
    ...
    <tbody>
      <tr>
        <td>Open Source</td>
        <td>Developers who want full control via self-hosting (no SaaS)</td>
        <td>
          <ul class="bullet-list">
            <li>MIT-licensed library, fully self-managed</li>
            <li>Unlimited usage (no built-in limits on nodes)</li>
          </ul>
        </td>
        <td>$0</td>
      </tr>
      <tr>
        <td>Developer</td>
        <td>Startups or hobby projects (low-volume usage)</td>
        <td>
          <ul class="bullet-list">
            <li>Self-hosted LangGraph server (free up to 100k nodes/mo included)</li>
            <li>Core orchestration features (task queues, state &amp; memory APIs, streaming)</li>
          </ul>
        </td>
        <td>
          $0 (100k nodes/mo free)<br>
          Pay-as-you-go when you exhaust limits
        </td>
      </tr>
      <tr>
        <td>Plus</td>
        <td>Teams needing managed cloud deployment</td>
        <td>
          <ul class="bullet-list">
            <li>All Developer features + cron scheduling, API auth, smart caching</li>
            <li>Fully managed SaaS (deploy from anywhere)</li>
          </ul>
        </td>
        <td>
          Usage-based ($0.001 per node + standby time);<br>
          $39/user/mo LangSmith subscription
        </td>
      </tr>
      <tr>
        <td>Enterprise</td>
        <td>Organizations needing custom deployment and support</td>
        <td>
          <ul class="bullet-list">
            <li>All Plus features + options for self-hosted or hybrid deployment</li>
            <li>Enterprise support (SLA, training, dedicated Slack channel)</li>
          </ul>
        </td>
...
<ul id=""><li id=""><strong id="">Debugging traces:</strong> Drill into every call in a chain or agent to spot wrong inputs, prompts, or model outputs fast.</li><li id=""><strong id="">Dataset collection:</strong> Capture prompts/response pairs to turn real traffic into a reusable test set.</li><li id=""><strong id="">Testing and evaluation:</strong> Run structured checks or regression tests against those datasets to prove changes work.</li><li id=""><strong id="">Prompt management:</strong> Version prompts, compare variants, and roll back when a change hurts quality.</li><li id=""><strong id="">Security and deployment:</strong> OAuth with Google and GitHub hosted in the US or EU; comes with basic admin roles.</li><li id="">Community Discord support only.</li></ul>

There are some limits that you also want to keep in mind:

<ul id=""><li id="">You get 1 seat and 1 personal workspace.</li><li id="">The free usage is limited to 5K base traces a month; pay $0.50 per 1K after that. Extended-retention traces cost $4.50 per 1K.</li><li id="">Throughput - up to 50K events per hour; 500 MB traces stored per hour.</li></ul>
...
<ul id=""><li id=""><strong id="">Managed cloud deployment</strong>: No need to host your own servers. LangChain runs and auto-scales the agent infrastructure for you in their cloud.</li><li id=""><strong id="">Cron scheduling</strong>: Schedule agents to run on intervals or at specific times via built-in cron triggers.</li><li id=""><strong id="">Authentication and authorization</strong>: Provides an auth layer so you can securely call LangGraph APIs from your applications.</li><li id=""><strong id="">Smart caching</strong>: Caches LLM calls to reduce redundant hits to the LLM API. In essence, if your agent asks the same question or performs the same step repeatedly, LangGraph can reuse previous results to save tokens.</li><li id=""><strong id="">LangGraph Studio (Cloud)</strong>: A web-based studio for prototyping and debugging agents, which is available in the Plus tier (the Developer tier has a local-only version).</li><li id=""><strong id="">Plus everything in the Developer plan:</strong> State management APIs, memory, streaming, etc., and support for up to 10 seats.</li></ul>
...
<ul id=""><li id=""><strong id="">Flexible deployment options:</strong> Enterprise customers can deploy LangGraph wherever they need – this includes fully self-hosted on your own cloud or on-premises, or a hybrid model (LangChain runs the control plane while your infrastructure handles the data plane).</li><li id=""><strong id="">Enterprise-grade support:</strong> LangGraph Enterprise comes with a formal Service Level Agreement (SLA) for uptime on managed offerings. You also get designated support staff, like a dedicated Customer Success Engineer, architectural guidance consultations, and even a shared Slack channel with the LangChain team for fast help.</li><li id=""><strong id="">Team training and onboarding:</strong> The Enterprise package includes training sessions to help your team use LangGraph effectively.</li><li id=""><strong id="">Enterprise admin features:</strong> Things like SSO integration (SAML/OKTA login), granular role-based access control, audit logging, and other compliance-related features typically come with the Enterprise plan.</li><li id=""><strong id="">Custom billing and procurement:</strong> Annual invoicing, ACH payments, vendor onboarding support – basically, the sales process is adapted to enterprise procurement needs.</li></ul>
...
<ul id=""><li id=""><strong id="">End-to-end LLMOps Toolkit:</strong> Ready-made toolkit with pre-built steps, templates, and guides for RAG pipelines, evaluation, reranking, finetuning, and more.</li><li id=""><a href="https://docs.zenml.io/concepts/steps_and_pipelines/scheduling" id=""><strong id="">Scheduling and Triggers</strong></a><strong id="">:</strong> Robust scheduling and triggers for pipelines automate ML workflows based on time or events, ensuring up-to-date models and efficient applications.</li><li id=""><strong id="">Smart Caching:</strong> Automatically caches pipeline step outputs, preventing redundant computations. Speeds up development iterations and reduces compute costs, especially during experimentation.</li></ul>
...
<ul id=""><li id=""><a href="https://www.zenml.io/llmops-database/multi-agent-system-for-prediction-market-resolution-using-langchain-and-langgraph" id="">**Chaos Labs</a>:** Edge AI Oracle used LangGraph for the multi-agent logic and ZenML for the surrounding LLMOps workflow.</li><li id=""><a href="https://www.zenml.io/llmops-database/building-a-scalable-conversational-video-agent-with-langgraph-and-twelve-labs-apis" id=""><strong id="">Jockey</strong></a>: Conversational Video Agent combined LangGraph graphs with ZenML infrastructure to scale video understanding pipelines.</li></ul>
...
<ul id=""><li id=""><a href="https://www.zenml.io/blog/prefect-pricing" id="">Prefect Pricing</a></li><li id=""><a href="https://www.zenml.io/blog/outerbounds-pricing" id="">Outerbounds Pricing</a></li><li id=""><a href="https://www.zenml.io/blog/wandb-pricing" id="">WandB Pricing</a></li><li id=""><a href="https://www.zenml.io/blog/clearml-pricing" id="">ClearML Pricing</a></li></ul>