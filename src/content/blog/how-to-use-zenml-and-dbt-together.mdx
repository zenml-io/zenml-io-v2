...
<ul id=""><li id="">Running a data transformation after training a model</li><li id="">Doing post-batch-inference data transformations (That’s Javier’s use case)</li><li id="">Triggering a training/inference/deployment ML workflow after a data transformation is complete</li></ul>
...
<div data-rt-embed-type="true"><pre><code fs-codehighlight-element="code">import requests
...
def trigger_dbt():
 ...
 &nbsp;else:
 &nbsp; &nbsp; &nbsp;return jsonify({'message': 'Failed to trigger GitHub Action'}), response.status_code</code></pre></div>
...
<div data-rt-embed-type="true"><pre><code fs-codehighlight-element="code">from dbt.cli.main import dbtRunner, dbtRunnerResult
...
# inspect the results
for r in res.result:
 &nbsp; &nbsp;print(f"{r.node.name}: {r.status}")</code></pre></div>
