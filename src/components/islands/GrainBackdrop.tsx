// GrainBackdrop — generic GrainGradient shader backdrop. Takes a GrainConfig
// palette; KitaruGrain (Kitaru landing) and the Labs homepage sections are
// thin callers. Behaviour (reveal gating, WebGL error boundary, SSR guard,
// reduced motion) lives here once.
//
...
import type { GrainConfig } from "../../lib/grainConfig";
...
export type GrainBackdropProps = {
  /** Palette + motion settings (hex only). */
  config: GrainConfig;
  /** Hover-revealed card mode: mounts the shader only while the parent
   *  `.group` is hovered/focused (hover-capable devices only). */
  hoverReveal?: boolean;
  class?: string;
  className?: string;
