/**
 * MLOps Database client-side filter island — "Research Hub" layout.
 *
 * Fetches /mlops-index.json on mount and provides:
 * - Left sidebar with faceted industry + technology navigation (desktop)
 * - Slide-out drawer for facets (mobile)
 * - Text search (title, company, platform, content type, summary)
 * - Multi-select mlops-tag filter with AND/OR toggle
...
interface ProcessedItem extends MLOpsIndexItem {
  searchText: string;
}
...
export interface MLOpsFilterProps {
  tags: FilterOption[];
...
const MOBILE_DRAWER_ID = "mlops-filters-drawer";
const MOBILE_DRAWER_TITLE_ID = "mlops-filters-drawer-title";
...
function getPagefind(): Promise<PagefindInstance | null> {
  ...
  pagefindPromise = import(/* @vite-ignore */ pagefindPath)
    ...
    .catch(() => {
      console.debug("[MLOps] Pagefind not available — using substring search");
      return null;
    });
  return pagefindPromise;
}

/** Extract slug from a Pagefind result URL like "/mlops-database/my-slug". */
...
function matchesTags(
  ...
  return mode === "and"
    ? selected.every((tag) => item.mlopsTags.includes(tag))
    : selected.some((tag) => item.mlopsTags.includes(tag));
}

function matchesIndustry(item: MLOpsIndexItem, industry: string): boolean {
...
export default function MLOpsFilter({
  tags,
  industries,
  pageSize = 24,
}: MLOpsFilterProps) {
  ...
  // Fetch data
  useEffect(() => {
    fetch("/mlops-index.json")
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      .then((data: MLOpsIndexItem[]) => {
  ...
  const tagCounts = useMemo(() => {
    ...
    const base = items.filter((item) => {
      if (usePagefind && pagefindSlugSet && !pagefindSlugSet.has(item.slug))
        return false;
    ...
    for (const item of base) {
      for (const tag of item.mlopsTags) {
  ...
  const renderFacets = (scope: FacetScope) => {
    const tagSearchId = `mlops-tag-search-${scope}`;
    ...
    return (
      <output class="flex items-center justify-center py-20">
        <div class="text-center">
          ...
          <p class="mt-4 text-sm text-gray-500">Loading MLOps database...</p>
        </div>
      </output>
    );
  }
      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div class="relative flex-1">
            ...
            <label for="mlops-search" class="sr-only">
              Search
            </label>
            <input
              id="mlops-search"
          ...
          {/* Sort dropdown */}
          <div class="sm:w-40">
            <label for="mlops-sort" class="sr-only">
              Sort
            </label>
            <select
              id="mlops-sort"
