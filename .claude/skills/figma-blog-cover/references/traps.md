...
- The loaded `figma:figma-use` skill's ops rule is at most 10 logical operations per `use_figma` call (a logical operation is creating a node, setting its properties, and parenting it; split larger work across calls). It does not exempt loops: **a loop over N nodes is N operations.** Read-only tree walks over hundreds of nodes ran without complaint — the rule is guidance, not an enforced quota — but writes respect it.
- S2 runs every time (it is idempotent and writes nothing when the column is right); it is the only step that moves the sections below a grown one, so S4 resizing a section is always followed by S2 → S5. S2: at most 10 ops per call — the create (if any) plus section moves; whatever does not fit comes back in `pending`; re-run S2 until it is `[]` (a new-month insert is one create plus one move per existing section of the column, so with nine sections it is one call at the budget). S4: same shape — a section resize (if any) plus child moves, `pending` for the rest (max 20 covers per section → two or three calls).
...
- Re-runs must converge: S1's `siblings` include the target's own live cover, so find-slot excludes `P.slug` from the used-photo set (and from the adjacency neighbours) — otherwise every re-run would rotate the background to the next free `Photo 0N` and S3 would rewrite the cover each time.
- `P` gets into a script through `scripts/figma/paste.mjs` (`node … paste.mjs <s*.js> P.json out.js`), which replaces the single `const P = __PARAMS__;` line and refuses a `P` with open `questions`. Use it as is — do not write a replacement helper, and never edit the pasted object.
...
- The image layouts set the title in an **800 px** measure — less than half of `Text` / `Full Bleed` (1680) and `Panel Bottom` (1760). Example: on `Image Right` the 24-character `Braintrust Pricing Guide` sets as `Braintrust Pricing / Guide`. find-slot predicts the breaks (`P.titleLines`) and names the wider legal layout; an orphan word on the last line is an export-blocking defect like a fourth line (SKILL.md S6).
- S3 updates an existing instance in place (copy, variant, background) — a copy fix never needs a delete or a new node.
- A `--comparison` run gets no cover-template headline question and no `titleLines` orphan suggestion: the VS card has no 800 px measure, and its budgets are the 34-character `vs.title` / `for` line. Its headline is derived as `<N> <Competitor> Alternatives` (N from the tiles, `Best` and `We Tested …` dropped); `--title` overrides a bad derivation.
...
- The VS recipe (`createInstance` from a `113:300` variant, `swapComponent` on a non-exposed nested instance) runs in one `s3-vs-comparison.js` call for `Count ≤ 7`. Retries reuse the instance by name; a broken instance is removed before retrying; a failure takes the `Panel Bottom` fallback and is reported — via a fresh `find-slot.ts <slug> --fallback "<reason>"` `P` (no `--comparison`), never by running the cover script with the VS `P` (its `expectedSetId` is `113:300` and S5 would reject the cover).
- Finding the month section and the `<slug>` instance already on the canvas (an aborted earlier attempt) is the normal idempotent case: S2 and S4 return `ops: 0, pending: []`, S3 rewrites the instance in place.
