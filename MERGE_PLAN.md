...
> **Status:** Phases 1, 2a, 2b, 2c, 3, 4, 5, 6, 7, 8, 11 landed on branch `merge/zenml-kitaru-unification`. Phase 10a is scaffolded. Phase 9 (unified homepage + redesign) is now the remaining design-blocked phase — scope expanded per D17: Zuri is doing a proper unified redesign later, and Phase 2c rolled the ZenML side back to its original purple brand in the meantime. Phase 10 (kitaru.ai 301s) is the remaining ops phase, blocked on 10a completion (D16).
...
| **3. Compare landing expansion** | ✅ Shipped (commits `d704f59`, `03a5825`, `6d829d9`, `910aab6`, `a4e652f`) | `merge/zenml-kitaru-unification` | Dual-collection `/compare` index with MLOps + Agents sections. 8 Kitaru competitor pages ported (`compare-kitaru` collection). scroll-reveal observer wired, Shiki theme issue fixed. |
| **4. `/get-started` ML/Agent chooser** | ✅ Shipped (MVP — design polish pending) | `merge/zenml-kitaru-unification` | `/get-started` is now a 2-card chooser: ML → `/get-started/zenml` (existing rich content moved one level down, intact), Agent → `/product/kitaru` (Kitaru landing already has install + quickstart). URL preserved (8.6k visitors/mo). Card surface tags: `data-analytics="GetStartedChoose:ML"` / `GetStartedChoose:Agent` for Plausible. Visual polish intentionally understated — Zuri can iterate. |
| **5. Kitaru blog migration** | ✅ Shipped (content + assets) | `merge/zenml-kitaru-unification` | All 11 Kitaru blog posts now on zenml.io (10 Kitaru-origin + `kitaru-launch.md` native). New `src/content/categories/kitaru.md` + `src/content/tags/kitaru.md`; all Kitaru-related posts re-tagged `category: "kitaru"` and `kitaru` prepended to tags array (previously all forced to `category: "zenml"` because `kitaru` didn't exist as a slug). `building-a-news-scout-on-kitaru` preserved as `draft: true`. Cover images + inline images mirrored to `zenml-assets` R2 bucket (kitaru-assets left intact for the live kitaru.ai site). |
| **6. Unified pricing** | ✅ Shipped (subtle merge per D7) | `merge/zenml-kitaru-unification` | Hero copy acknowledges both workspaces. New `PRICING_WORKSPACES` block renders 2-card workspace explainer between hero and pricing tabs, linking to `/get-started/zenml` and `/product/kitaru`. 2 Kitaru-aware FAQ entries added. Tier structure unchanged — same $ per D7. |
| **7. `/pro` unified pitch** | ✅ Shipped | `merge/zenml-kitaru-unification` | Hero rewritten as "managed control plane for ML pipelines and AI agents". New `PRO_UNIFIED_PITCH` block (D10 split): "Managed control plane" section covers both workspaces; "Enterprise governance" section covers SSO/RBAC/audit/compliance/SLA. 1 new Kitaru-aware FAQ entry. Page surface flipped to `unified`. |
...
| **2c. ZenML brand revert (Zuri's call)** | ✅ Shipped (commits `14c39be`, `9bcf6bf`, `3b9ca12`) | `merge/zenml-kitaru-unification` | Reverted the ZenML side from sage-green/cream/Inter back to the original purple/white/Plus Jakarta Sans identity per Zuri (D17). `[data-app="zenml"]` block in `global.css` now maps to `#7a3ef4` primary, `#ffffff` ground, `#111827` foreground, Plus Jakarta Sans. `--color-zenml-*` ladder restored to original hex (`#f1ebfe → #7a3ef4 → #1b0c3b`). Assets reverted: `gradient_01.webp`, `grid_bg_02.webp`, `tab_bg.avif`, `zenml_light_bg-02.avif`, `hero-0925.json`. `why-zenml.avif` local copies deleted (R2 originals re-pointed via `homepage.ts`). 7 dark-CTA sections (FinalCTA, WhitepaperCTA, FeaturesCTA05, FeaturesHubCTA, VsCta02, /deployments, /projects) rolled back from `MeshGradientBg` to the original static `gradient_01.webp` backdrop. **Kitaru side untouched** — `:root` defaults + `[data-app="kitaru"]` keep the warm cream/orange identity. |
| 9. Unified homepage + redesign | 🚧 Blocked | — | Scope expanded per D17: Zuri is doing a proper unified redesign later (not just a token tweak). Until then, ZenML side stays on original purple, Kitaru side stays on warm cream/orange. `/product/zenml` scaffold + `homepage-unified.ts` copy draft already on the branch — both wait for the redesign before going live. |
...
| **10. kitaru.ai → 301 redirects** | ✅ Shipped (2026-05-27) | — | 27 literal URLs deployed via Cloudflare Bulk Redirects (account-level list `kitaru_ai_redirects` + entrypoint ruleset on `http_request_redirect` phase). `/docs/*` preserved on kitaru.ai per D2. www.kitaru.ai → kitaru.ai redirect preserved. No catch-all — per Plausible audit, virtually zero traffic to non-listed URLs (better SEO to 404 than redirect unknowns to wrong page). Query strings preserved on all rules. Tested end-to-end. |
...
- ~~**Segment write key hardcoded to ZenML**~~ — **Resolved differently than originally planned.** D4 superseded: rolled back to single ZenML write key once D5 was reversed (the only consumer of the Kitaru key would have been the deleted routes). `consentConfig.ts` Segment snippet now uses one key; the page-init call passes `{surface}` as a property so the dimension still flows to downstream tools.
- ~~**`/api/{get-started,waitlist,newsletter}` endpoints not ported**~~ — **Resolved by removal.** D5 superseded after audit: the three routes had zero callers in the merged repo, so the port produced dead code. Routes + supporting libs (`src/lib/kitaru-{segment,form-types}.ts`) deleted; no deployment-side KV/secret/binding work required.
...
- ~~**PlatformBuilder why-card images hotlink to `assets.kitaru.ai`**~~ — **DONE.** All 30 unique R2 keys referenced under `assets.kitaru.ai` (blog covers, compare OG cards, competitor logos, PlatformBuilder why-cards, Features bg, kitaru-agents-now-have-memory inline images, claude-agent-sdk feature graphics) **copied** (not moved) from `kitaru-assets` → `zenml-assets` preserving the same key paths. Source rewritten to `assets.zenml.io/...`. `kitaru-assets` bucket left intact so the live kitaru.ai site keeps working until the DNS cutover.
...
| ~~**D4**~~ | ~~**Segment:** keep two write keys.~~ **Superseded 2026-05-17.** Audit found zero callers in the merged site for the Kitaru-side write key (the routes that needed it turned out to be dead code, see D5 below). Consolidated to one Segment workspace; surface still flows as a property on `analytics.page()` so downstream segmentation works without the two-key split. |
| ~~**D5**~~ | ~~Form endpoints from kitaru port **as-is**.~~ **Superseded 2026-05-17.** All three routes (`/api/get-started`, `/api/waitlist`, `/api/newsletter`) had zero callers in the merged repo — Kitaru landing CTAs already point to `/book-your-demo` (ZenML's Cal.com flow), ZenML's BrevoNewsletterForm handles newsletter, and "waitlist" stopped being a concept once Kitaru launched open-source. Routes + supporting libs deleted; the three KV namespaces that would have backed them stay on the Cloudflare account but aren't bound to the Pages project. |
...
| **D17** | **Revert ZenML side to original purple brand. Defer a proper unified redesign to Phase 9 (Zuri-led).** D15's site-wide sage/cream/Inter swap is rolled back on the ZenML side only; Kitaru side keeps its warm cream/orange identity. The `[data-app="zenml" \| "kitaru"]` mechanism, the Kitaru landing port, the Kitaru compare pages, and the Kitaru-themed blog posts all stay intact. | Zuri's call (2026-05-18): "do a proper redesign later, first make the ZenML colours back as the original website." The provisional sage swap was mixing with production content in a way that didn't read as either brand cleanly. Cleaner to operate the two sides on their established identities until the unified redesign actually lands. |
...
- [ ] Phase 9 trigger: when does Zuri deliver the unified redesign (post-D17, scope is now a full redesign — tokens + page layouts, not just colors)?
- [ ] Audit remaining sage/green leaks after Phase 2c (one already found and fixed: `zenml_light_bg-02.avif`). Run another pass once Codex / a designer eyes the rendered site against live `zenml.io`.
