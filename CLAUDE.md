...
- `src/components/templates/` — the Wave 2 section families (#249), built on the substrate and registered in `src/lib/templates/registry.ts`: `PageHeader` (six collapsing arrangements incl. tinted band + split-masthead), `ProcessSteps` (`vertical-code` + `compact-list`, semantic `<ol>`), `MetadataBlock`/`SpecTable`/`DescriptionList`/`StackedList` (data-display), `RelatedRail` (related-content items arrangement — static, not an island), `TermHubEditorial`/`TermHubEntryIndex`/`TermHubCatalog` (term-hub shapes), plus `chipStyles.ts` (shared chip color variants; plain `.ts` helpers are exempt from `check:registry`). Live routes render current-brand parity arrangements; the drawn new-brand states (zero-padded numerals, leading meta separators, stacked entry lists, sticky spec-table first column, hex-corner cards) exist as demo/cutover paths and are exercised on `/styleguide` via registry `demoProps` — each deviation is documented in the entry's `notes`
