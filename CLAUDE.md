...
- `src/components/kitaru/islands/*` — Preact landing islands (Hero, ScenarioStrip, OneImport, Importers, CodeShowcase, AgentDriven, SocialProof, KitaruGrain WebGL shader) plus shared helper modules (the authoritative list is `KITARU_ISLAND_HELPERS` in `scripts/check-dist-smoke.ts`). The seven sections mount `client:visible` from `product/kitaru.astro`; hydration is covered by `pnpm check:islands` (OneImport check) and the `check-dist-smoke.ts` island manifest
