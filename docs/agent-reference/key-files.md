...
- `src/styles/global.css` §2b-ii — the `[data-app="kitaru"]` wrapper scope: the token names the Kitaru components read, pointed at the rebrand tokens (surfaces from the labs semantic set, ember → orange-600, night → cream-900, Borna/Rethink/Nudica type roles), which is how `/product/kitaru` and the Kitaru compare pages render in the Labs shell without a class rename. The old `kitaru-compat.css` sheet is gone
...
- `src/pages/product/kitaru.astro` — Kitaru landing (Aug 2026 redesign; copy lives in `src/lib/kitaru-landing.ts`, CTA links in `src/lib/productKitaru.ts`), rendered in the Labs shell (`app="labs" product="kitaru"`, orange-600 signup pill, `KitaruLockup` brand link). `<main>` carries `overflow-x-clip` as the DESIGN.md guard for the reveal-left/right slides; its hero and close bands follow the product-page CTA spec (Start free pill, ghost Read the docs on the hero, the install chip on its own row beneath) so they match `/product/zenml`
- `src/components/kitaru/*` — landing section shells (Hero, FirstRun, Features, Faq, Cta, `_HighlightPanel`). Hero is a static full-viewport band (`min-h-svh`, content centered below the nav overlay) that renders the copy, the `LabsButton` pair (signup, ghost docs) and `LabsInstallChip` beneath; Cta uses the same two Labs primitives (orange pill, chip beneath). Hero and Cta mount `KitaruGrain` directly as a standalone island for their shader backdrops; Features gets the same via the `_HighlightPanel` shells it renders. Grain palettes come from `src/lib/kitaru-grain-palettes.ts` (rebrand orange ramp)
...
- `src/scripts/kitaru/*` — Kitaru-page client scripts (clipboard, reveal-static); `src/hooks/use-reveal.ts` is the Preact-island counterpart of reveal-static
...
`ZENML.IO` share a row 120 px down, the copy starts on the midline, and title
