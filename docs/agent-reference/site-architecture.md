...
- `src/` contains the Astro app:
  ...
  - `content/` markdown CMS content, validated by the collection definitions in `src/content.config.ts`. Three compare collections: `compare/` (legacy ZenML-vs-X, Webflow-era `.md`), `compare-zenml/` (ZenML-vs-X in the MDX template: durable execution engines and agent frameworks) and `compare-kitaru/` (Kitaru-vs-X, agent frameworks Kitaru has adapters for).
